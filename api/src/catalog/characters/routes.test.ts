...
describe('character routes (franchise-scoped)', () => {
  ...
  describe('GET /catalog/franchises/:franchise/characters/:slug', () => {
    it('should return 200 with full character detail', async () => {
      // base query, then Promise.all([sub-groups, appearances])
      mockQuery
    ...
    it('should return is_combined_form flag for combined form characters', async () => {
      const gestaltRow = {
        ...charBaseRow,
        name: 'Devastator',
        slug: 'devastator',
      ...
      expect(res.statusCode).toBe(200);
      const body = res.json<{ is_combined_form: boolean }>();
