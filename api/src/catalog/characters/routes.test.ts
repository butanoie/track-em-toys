...
describe('character routes (franchise-scoped)', () => {
  ...
  const charListRow = {
    id: 'c-1', name: 'Optimus Prime', slug: 'optimus-prime',
    franchise_slug: 'transformers', franchise_name: 'Transformers',
    faction_slug: 'autobot', faction_name: 'Autobot',
    continuity_family_slug: 'g1', continuity_family_name: 'Generation 1',
    character_type: 'Transformer', alt_mode: 'semi-truck',
  ...
  const charBaseRow = {
    ...charListRow,
    combiner_role: null,
    combined_form_id: null, combined_form_slug: null, combined_form_name: null,
  ...
  describe('GET /catalog/franchises/:franchise/characters', () => {
    it('should return 200 with paginated characters', async () => {
      ...
      mockQuery
        .mockResolvedValueOnce({ rows: [charListRow] })
        .mockResolvedValueOnce({ rows: [{ total_count: 1 }] })

      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/transformers/characters?limit=20',
      })
    ...
    it('should return next_cursor when more results exist', async () => {
      ...
      mockQuery
        ...
        .mockResolvedValueOnce({ rows: [{ total_count: 10 }] })

      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/transformers/characters?limit=2',
      })
      expect(res.statusCode).toBe(200)
    ...
    it('should return empty data for franchise with no characters', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/gi-joe/characters',
      })
      expect(res.statusCode).toBe(200)
  ...
  describe('GET /catalog/franchises/:franchise/characters/:slug', () => {
    it('should return 200 with full character detail', async () => {
      ...
      mockQuery
        ...
        .mockResolvedValueOnce({ rows: [{
          id: 'a-1', slug: 'g1-cartoon', name: 'G1 Cartoon',
          source_media: 'TV', source_name: 'The Transformers Season 1',
          year_start: 1984, year_end: 1985, description: null,
        }] })

      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/transformers/characters/optimus-prime',
      })
      expect(res.statusCode).toBe(200)
      const body = res.json<{
        name: string
    ...
    it('should return 404 when character not found', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/transformers/characters/nonexistent',
      })
    ...
    it('should return 404 when character exists in different franchise', async () => {
      mockQuery.mockResolvedValueOnce({ rows: [] })
      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/gi-joe/characters/optimus-prime',
      })
      expect(res.statusCode).toBe(404)
    })
    ...
    it('should handle character with null faction and no sub-groups', async () => {
      const noFactionRow = {
        ...charBaseRow,
        faction_slug: null, faction_name: null,
      }
      mockQuery
    ...
    it('should include combined_form when character is a combiner component', async () => {
      const componentRow = {
        ...charBaseRow,
        name: 'Scrapper', slug: 'scrapper',
        combiner_role: 'right leg',
        combined_form_slug: 'devastator', combined_form_name: 'Devastator',
      }
      mockQuery
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/franchises/transformers/characters/scrapper',
      })
      expect(res.statusCode).toBe(200)
