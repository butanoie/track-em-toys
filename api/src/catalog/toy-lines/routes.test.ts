...
describe('toy-line routes (franchise-scoped)', () => {
  ...
  const toyLineRow = {
    ...
    scale: '1:24',
    description: 'Premium collector line',
  ...
  describe('GET /catalog/franchises/:franchise/toy-lines', () => {
    it('should return 200 with toy line list', async () => {
      ...
      const res = await server.inject({
        method: 'GET',
        url: '/catalog/franchises/transformers/toy-lines',
      });
      expect(res.statusCode).toBe(200);
      const body = res.json<{
        data: Array<{ franchise: { slug: string }; manufacturer: { slug: string; name: string } }>;
      }>();
  ...
  describe('GET /catalog/franchises/:franchise/toy-lines/:slug', () => {
    it('should return 200 with detail', async () => {
      ...
      const res = await server.inject({
        method: 'GET',
        url: '/catalog/franchises/transformers/toy-lines/masterpiece',
      });
      expect(res.statusCode).toBe(200);
    });
    ...
    it('should return 404 when not found', async () => {
      ...
      const res = await server.inject({
        method: 'GET',
        url: '/catalog/franchises/transformers/toy-lines/nonexistent',
      });
