  ...
  describe('GET /catalog/search', () => {
    it('should return 200 with search results', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/search?q=optimus',
      })
      expect(res.statusCode).toBe(200)
      const body = res.json<{
        data: Array<{ entity_type: string; franchise: { slug: string } }>
        page: number; limit: number; total_count: number
      }>()
    ...
    it('should support franchise filter', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/search?q=optimus&franchise=transformers',
      })
      expect(res.statusCode).toBe(200)
    ...
    it('should support pagination params', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/search?q=test&page=3&limit=5',
      })
      expect(res.statusCode).toBe(200)
    ...
    it('should return empty results for no matches', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/search?q=zzzznonexistent',
      })
      expect(res.statusCode).toBe(200)
    ...
    it('should return empty results for punctuation-only query', async () => {
      ...
      const res = await server.inject({
        method: 'GET', url: '/catalog/search?q=!!!',
      })
      expect(res.statusCode).toBe(200)
      const body = res.json<{ data: unknown[]; total_count: number }>()
      expect(body.data).toHaveLength(0)
