...
describe('queries', () => {
  ...
  describe('createUser', () => {
    ...
    it('should handle null optional fields', async () => {
      ...
      vi.mocked(client.query).mockResolvedValue(
        mockQueryResult([{ ...mockUser, email: null, display_name: null, avatar_url: null }], 1)
      );
  ...
  describe('updateUserDisplayName', () => {
    it('should update display name only when currently null', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('WHERE id = $1 AND display_name IS NULL'), [
        'user-1',
        'New Name',
      ]);
    });
  ...
  describe('getUserStatusAndRole', () => {
    it('should return active status and role for non-deactivated, non-deleted user', async () => {
      // safe: mockResolvedValue is typed for the query's return shape
      vi.mocked(client.query).mockResolvedValue(
        mockQueryResult([{ role: 'user', deactivated_at: null, deleted_at: null }], 1)
      );
    ...
    it('should return deactivated status and role for deactivated user', async () => {
      // safe: mockResolvedValue is typed for the query's return shape
      vi.mocked(client.query).mockResolvedValue(
        mockQueryResult([{ role: 'curator', deactivated_at: '2026-01-15T00:00:00Z', deleted_at: null }], 1)
      );
    ...
    it('should return deleted status and role for GDPR-purged user', async () => {
      // safe: mockResolvedValue is typed for the query's return shape
      vi.mocked(client.query).mockResolvedValue(
        mockQueryResult([{ role: 'user', deactivated_at: null, deleted_at: '2026-01-20T00:00:00Z' }], 1)
      );
  ...
  describe('findOAuthAccountsByUserId', () => {
    it('should return all accounts for a user', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('FROM oauth_accounts WHERE user_id = $1'), [
        'user-1',
      ]);
    });
  ...
  describe('createRefreshToken', () => {
    it('should insert and return the refresh token record', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO refresh_tokens'), [
        'user-1',
        'abc123hash',
        'Chrome/120',
        expiresAt,
  ...
  describe('deleteOrphanUser', () => {
    it('should execute DELETE with NOT EXISTS guard using the correct SQL and parameter', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM users WHERE id = $1'), ['user-1']);
      const sql = vi.mocked(client.query).mock.calls[0]?.[0] as string | undefined;
      expect(sql).toBeDefined();
    ...
    it('should propagate DB errors to the caller', async () => {
      ...
      await expect(deleteOrphanUser(client, 'user-1')).rejects.toThrow('connection terminated unexpectedly');
    });
  });
  ...
  describe('revokeRefreshToken', () => {
    it('should set revoked_at to NOW() for the given token hash', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith('UPDATE refresh_tokens SET revoked_at = NOW() WHERE token_hash = $1', [
        'token-hash',
      ]);
    });
  ...
  describe('revokeAllUserRefreshTokens', () => {
    it('should revoke all active tokens for a user', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('WHERE user_id = $1 AND revoked_at IS NULL'), [
        'user-1',
      ]);
    });
  });
  ...
  describe('logAuthEvent', () => {
    it('should insert an auth event with metadata', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO auth_events'), [
        'user-1',
        'signin',
        '192.168.1.1',
        'Test Agent',
