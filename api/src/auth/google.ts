 ...
 */
export async function verifyGoogleToken(idToken: string): Promise<ProviderClaims> {
  ...
  if (
    (config.google.iosClientId && audList.includes(config.google.iosClientId)) ||
    ...
    throw new ProviderVerificationError(
      `Google id_token audience "${audList.join(', ')}" does not match any configured client ID`
    );
  }
