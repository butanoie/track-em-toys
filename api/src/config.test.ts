...
describe('config', () => {
  ...
  describe('requiredPem — newline replacement', () => {
    it('replaces literal \\n sequences in JWT_PRIVATE_KEY with real newlines', async () => {
      ...
      const pemWithLiteralNewlines = '-----BEGIN EC PRIVATE KEY-----\\nABCDEFG\\n-----END EC PRIVATE KEY-----';
      ...
      expect(config.jwt.privateKey).toBe('-----BEGIN EC PRIVATE KEY-----\nABCDEFG\n-----END EC PRIVATE KEY-----');
    });

    it('leaves PEMs that already contain real newlines unchanged', async () => {
      const pemWithRealNewlines = '-----BEGIN EC PRIVATE KEY-----\nABCDEFG\n-----END EC PRIVATE KEY-----';
  ...
  describe('PORT parsing', () => {
    ...
    it('throws when PORT=abc is not a valid port number', async () => {
      vi.stubEnv('PORT', 'abc');
      await expect(import('./config.js')).rejects.toThrow('PORT must be a number between 1 and 65535, got: abc');
    });

    it('throws when PORT=0 is out of range', async () => {
      vi.stubEnv('PORT', '0');
      await expect(import('./config.js')).rejects.toThrow('PORT must be a number between 1 and 65535, got: 0');
    });

    it('throws when PORT=65536 exceeds max', async () => {
      vi.stubEnv('PORT', '65536');
      await expect(import('./config.js')).rejects.toThrow('PORT must be a number between 1 and 65535, got: 65536');
    });
  ...
  describe('DB_POOL_MAX parsing', () => {
    ...
    it('throws when DB_POOL_MAX is not a number', async () => {
      ...
      await expect(import('./config.js')).rejects.toThrow('DB_POOL_MAX must be a number between 1 and 1000, got: abc');
    });

    it('throws when DB_POOL_MAX is 0 (below minimum)', async () => {
      vi.stubEnv('DB_POOL_MAX', '0');
      await expect(import('./config.js')).rejects.toThrow('DB_POOL_MAX must be a number between 1 and 1000, got: 0');
    });

    it('throws when DB_POOL_MAX is 1001 (above maximum)', async () => {
      vi.stubEnv('DB_POOL_MAX', '1001');
      await expect(import('./config.js')).rejects.toThrow('DB_POOL_MAX must be a number between 1 and 1000, got: 1001');
    });
  ...
  describe('Apple Sign-In required variables', () => {
    it.each(['APPLE_TEAM_ID', 'APPLE_KEY_ID', 'APPLE_PRIVATE_KEY', 'APPLE_BUNDLE_ID', 'APPLE_SERVICES_ID'])(
      'throws when %s is missing',
      async (varName) => {
        vi.stubEnv(varName, '');
        await expect(import('./config.js')).rejects.toThrow(`Missing required environment variable: ${varName}`);
      }
    );

    it('replaces literal \\n sequences in APPLE_PRIVATE_KEY with real newlines', async () => {
      const pemWithLiteralNewlines = '-----BEGIN PRIVATE KEY-----\\nABCDEFG\\n-----END PRIVATE KEY-----';
      ...
      expect(config.apple.privateKey).toBe('-----BEGIN PRIVATE KEY-----\nABCDEFG\n-----END PRIVATE KEY-----');
    });
  ...
  describe('Google Sign-In required variables', () => {
    it.each(['GOOGLE_WEB_CLIENT_ID', 'GOOGLE_IOS_CLIENT_ID'])('throws when %s is missing', async (varName) => {
      vi.stubEnv(varName, '');
      await expect(import('./config.js')).rejects.toThrow(`Missing required environment variable: ${varName}`);
    });
