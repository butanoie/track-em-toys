...
export default defineConfig({
  ...
  projects: [
    ...
    // Authenticated as regular user — catalog browsing, session tests
    {
      ...
      use: { ...devices['Desktop Chrome'] },
      testMatch:
        /authenticated-session\.spec\.ts|catalog-browse\.spec\.ts|catalog-detail-pages\.spec\.ts|catalog-search\.spec\.ts|session-persistence\.spec\.ts|collection\.spec\.ts|collection-export-import\.spec\.ts|add-by-photo\.spec\.ts|collection-photos\.spec\.ts/,
    },
