...
import { uploadCollectionPhoto, validateFile, DuplicateUploadError } from './api';
...
interface UseCollectionPhotoUploadOptions {
  collectionItemId: string;
  onUploadComplete: () => void;
}

export interface UseCollectionPhotoUploadReturn {
  items: UploadItem[];
  isUploading: boolean;
...
export function useCollectionPhotoUpload({
  collectionItemId,
  onUploadComplete,
}: UseCollectionPhotoUploadOptions): UseCollectionPhotoUploadReturn {
  ...
  useEffect(() => {
    ...
    uploadCollectionPhoto(collectionItemId, file, (p) => {
  ...
  }, [items, collectionItemId, onUploadComplete]);
